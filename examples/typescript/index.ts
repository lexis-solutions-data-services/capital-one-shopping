...
async function runActor() {
  ...
  const actorId = `lexis-solutions/capital-one-shopping`;
  console.log(`Starting actor: ${actorId}`);

  const { defaultDatasetId } = await client.actor(actorId).call({
    startUrls: [
      {
        "url": "https://capitaloneshopping.com/search?q=eyJzaXplIjo1MCwibWF0Y2giOiJuaWtlIiwiZnJvbSI6MCwiZmlsdGVycyI6eyJicmFuZHMiOlsiTmlrZSJdfSwic29ydCI6eyJwcmljZSI6ImRlc2MifX0%3D"
      }
    ],
    model: "PRODUCT",
    maxItems: 20,
